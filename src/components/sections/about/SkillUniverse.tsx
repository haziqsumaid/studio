...
const SkillBadge = ({ skill, angle, radius, index }: { skill: Skill; angle: number; radius: number; index: number }) => {
  ...
  const initialX = radius * 1.5 * Math.cos(angle); // Start further out
  const initialY = radius * 1.5 * Math.sin(angle); // Start further out
  ...
  const progressRingVariants = {
    hidden: { pathLength: 0 },
    visible: {
      pathLength: reducedMotion ? skill.proficiency / 100 : skill.proficiency / 100,
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate={controls}
      whileHover={!reducedMotion ? { scale: 1.15, zIndex: 10, boxShadow: "0px 0px 20px hsla(var(--primary), 0.5)" } : {}}
      className="absolute flex flex-col items-center justify-center w-24 h-24 md:w-28 md:h-28 rounded-full bg-card/50 backdrop-blur-sm border border-border/30 shadow-md p-2 cursor-pointer"
      style={{ 
        left: `calc(50% - 3rem + ${x}px)`, // Adjust for badge size
        top: `calc(50% - 3rem + ${y}px)`,  // Adjust for badge size
       }}
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        <div className="text-primary mb-1 scale-125 group-hover:text-[hsl(var(--gradient-end))] transition-colors">
          {skill.icon}
        </div>
...
export function SkillUniverse({ skills }: SkillUniverseProps) {
  const radius = 150; // Adjust radius for screen size if needed
      ...
      <div className="relative w-full aspect-square max-w-lg mx-auto flex items-center justify-center">
        ...
        {skills.map((skill, index) => (
          <SkillBadge
            ...
            radius={reducedMotion ? radius * 0.8 : radius} // Smaller radius if reduced motion to avoid overlap
            index={index}
          />
        ))}
      </div>
